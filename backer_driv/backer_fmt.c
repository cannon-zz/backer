...
static unsigned int  bkr_find_sector(unsigned long);
...
unsigned char  weight[] = WEIGHT_INITIALIZER;   /* weight[x1 ^ x2] == anti-correlation weight */
...
int  bkr_set_parms(unsigned int mode, unsigned int max_buffer)
{
	...
	switch(BKR_FORMAT(device.mode))
		...
		case BKR_RAW:
		/*
		 * For debugging.  Uncommenting this makes the DMA buffer a
		 * little bit smaller than an integer number of video
		 * frames so that a buffer overrun can be seen as a shift
		 * in the pattern generated by bkrtest in raw mode.
		 */
 ...
 * and tail offsets for the device buffer since these need to have certain
 * relationships with respect to one another in order to force the sector
 * layer to update them upon the first read/write operation.  NOTE: this
 * has no bearing on the actual I/O transfers which must always start at 0,
 * this is done merely to force the sector layer to start at 0 as well.
 */
...
static unsigned int bkr_find_sector(unsigned long bailout)
{
	unsigned int  i, j, bad_bits;
	unsigned int  skipped = -1;

	bkr_device_read(sector.footer_offset + SECTOR_KEY_OFFSET, bailout);
	...
	do
		...
		if(i >= device.size)
			...
			while(i >= device.bytes_per_line)
				bad_bits += weight[device.buffer[i -= device.bytes_per_line] ^ key[--j]];
			i += device.size;
			}
		while(j)
			bad_bits += weight[device.buffer[i -= device.bytes_per_line] ^ key[--j]];
